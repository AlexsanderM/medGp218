...
public class Const {
    ...
    // public final static String SEARCH_ID_IN_SERVICES = "SELECT idServices, dateService, diagnoz, service, serPolis FROM " + Const.SERVICESTABLE + " WHERE " + Const.SERVICE_DATE + " =? AND " +
    //         Const.SERVICE_SERPOLIC  + "=? AND " + Const.SERVICE_DIAGNOZ + "=? AND " + Const.SERVICE_SERVICE + "=? LIMIT 1";
    public final static String SEARCH_ID_IN_SERVICES = "SELECT idServices, dateService, diagnoz, service, serPolis FROM " + Const.SERVICESTABLE + " WHERE " + Const.SERVICE_DATE + " >= '2018-02-01' AND " +
            Const.SERVICE_DATE + "<= '2018-02-31' ";
    ...
    public final static String SEARCH_ERROR_OVER_IN_SERVICES = "SELECT idServices, dateService, diagnoz, serPolis, service, kratnost, fioDocter, moEntity, fioPathient, sex  FROM " + Const.SERVICESTABLE + " WHERE " + Const.SERVICE_DATE + " >=? AND " +
            Const.SERVICE_DATE + "<=?" + " AND (" + Const.SERVICE_SERVICE + "< 25001 OR " + Const.SERVICE_SERVICE + "> 30111) AND " + Const.SERVICE_SERVICE + "<> 1571 AND " + Const.SERVICE_SERVICE + "<> 1581 AND " + Const.SERVICE_SERVICE + "< 125001";
